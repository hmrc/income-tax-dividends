...
class StockDividendsUserDataRepositoryISpec extends IntegrationTest with FutureAwaits with DefaultAwaitTimeout {
  ...
  "find" should {
    ...
    "return an empty model when no data found" in {
      count mustBe 1
      val dataAfter: Option[StockDividendsUserDataModel] = await(stockDividendsRepo.find(taxYear - 1)(testUser).map {
        case Right(value) => value
        case Left(_) => None
      })
  ...
  "the set indexes" should {
    ...
    "enforce uniqueness" in {
      ...
      result.left.e.swap.getOrElse(new Exception("wrong message")).getMessage must include(
        "E11000 duplicate key error collection: income-tax-dividends.stockDividendsUserData index: UserDataLookupIndex dup key:")
    }
