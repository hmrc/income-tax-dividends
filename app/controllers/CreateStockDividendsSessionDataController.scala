...
class CreateStockDividendsSessionDataController @Inject()(stockDividendsIncomeDataService: StockDividendsSessionService,
  ...
  def createSessionData(taxYear: Int): Action[AnyContent] = authorisedAction.async { implicit user =>
    user.body.asJson.map(_.validate[StockDividendsCheckYourAnswersModel]) match {
      case Some(JsSuccess(model, _)) =>
        stockDividendsIncomeDataService.createSessionData(model, taxYear)(NotModified)(NoContent) // review NotModified error.. should be internalServerError
