...
class CreateUpdateStockDividendsIncomeController @Inject()(createUpdateDividendsIncomeService: CreateUpdateDividendsIncomeService,
                                                           cc: ControllerComponents,
                                                           authorisedAction: AuthorisedAction)
                                                          (implicit ec: ExecutionContext) extends BackendController(cc) {

  def createUpdateStockDividends(nino: String, taxYear: Int): Action[AnyContent] = authorisedAction.async { implicit user =>
