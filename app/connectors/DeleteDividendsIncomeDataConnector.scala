...
class DeleteDividendsIncomeDataConnector @Inject()(http: HttpClient, val appConfig: AppConfig)(implicit ec: ExecutionContext) extends IFConnector {
  def deleteDividendsIncomeData(nino: String,
    ...
    val deleteDividendsIncome: String = appConfig.ifBaseUrl + s"/income-tax/income/dividends/$nino/$taxYearParameter"

    http.DELETE[DeleteDividendsIncomeResponse](deleteDividendsIncome)(DeleteDividendsIncomeHttpReads,
      ifHeaderCarrier(deleteDividendsIncome, DeleteDividendsIncome), ec)
  }
}
